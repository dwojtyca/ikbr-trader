...
function groupCandles(candles: Candle[]): Map<string, Candle[]> {
  ...
  for (const [key, rows] of out.entries()) {
    out.set(key, rows.sort((a, b) => a.ts.getTime() - b.ts.getTime()));
  }
  return out;
}
...
export class BacktestSimulator {
  ...
  async run(progress?: RunProgressOptions): Promise<{ totalPnl: number; trades: number; wins: number; winRate: number }> {
    ...
    const events = this.data.candles1m
      ...
      .sort((a, b) => a.candle.ts.getTime() - b.candle.ts.getTime() || a.key.localeCompare(b.key));
    ...
    await progress?.onProgress?.({ current: progressBase, total: progressTotal, label: progressLabel });
    ...
    for (let eventIndex = 0; eventIndex < events.length; eventIndex += 1) {
      ...
      if (eventIndex > 0 && eventIndex % PROGRESS_EVERY_EVENTS === 0) {
        await progress?.onProgress?.({ current: progressBase + eventIndex, total: progressTotal, label: progressLabel });
      }
    ...
    return {
      ...
      wins,
      winRate: this.closedTrades.length > 0 ? wins / this.closedTrades.length : 0
    };
  }
  ...
  async getRecentCandles(symbol: string, timeframe: Candle['timeframe'], limit: number): Promise<Candle[]> {
    ...
    const index = binarySearchLastAtOrBefore(rows, this.currentTime ?? new Date(0));
    if (index < 0) return [];
    return rows.slice(Math.max(0, index - limit + 1), index + 1);
  }
  ...
  async getExposureSnapshot(): Promise<any> {
    ...
    for (const [symbol, position] of this.positions.entries()) {
      ...
      const fxToBase = this.fxToBaseForSymbol(symbol, this.currentTime ?? new Date()) ?? position.fxToBaseAtEntry;
      const marketValue = position.quantity * price * priceMultiplier * fxToBase;
      ...
      positionContextsBySymbol[symbol] = {
        ...
        marketPrice: price,
        marketValue,
        unrealizedPnL: position.quantity > 0
          ? (price - position.averageCost) * Math.abs(position.quantity) * priceMultiplier * fxToBase
          : (position.averageCost - price) * Math.abs(position.quantity) * priceMultiplier * fxToBase
      };
    }
  ...
  async getStrategyRuntimeState(strategyId: string): Promise<StrategyState> {
    ...
    if (state.cooldownUntil && this.currentTime) {
      const cooldownActiveInSimulatedTime = state.cooldownUntil.getTime() > this.currentTime.getTime();
      return {
        ...state,
        cooldownUntil: cooldownActiveInSimulatedTime ? new Date(Date.now() + 60_000) : undefined
      };
    }
  ...
  async getSymbolNetPnlSince(symbol: string, since: Date): Promise<number> {
    ...
    return this.closedTrades
      .filter((trade) => trade.instrument.toUpperCase() === key && trade.exitedAt >= since)
      .reduce((sum, trade) => sum + trade.pnl, 0);
  }

  async getSignalPerformance(input: { instrument?: string; strategy: string; side: Side; limit: number }): Promise<any> {
    const rows = this.closedTrades
      .filter((trade) =>
        trade.strategy === input.strategy &&
        trade.side === input.side &&
        (!input.instrument || trade.instrument.toUpperCase() === input.instrument.toUpperCase())
      )
      .slice(-input.limit);
    ...
    return {
      ...
      avgPnlPct: pnlPcts.length ? pnlPcts.reduce((sum, value) => sum + value, 0) / pnlPcts.length : undefined,
      medianPnlPct: this.median(pnlPcts),
      expectancyPct: pnlPcts.length ? pnlPcts.reduce((sum, value) => sum + value, 0) / pnlPcts.length : undefined
    };
  }
  ...
  private async fillOrder(pending: PendingOrder, candle: Candle): Promise<void> {
    ...
    if (!existing || Math.sign(existing.quantity) === Math.sign(qty)) {
      ...
      if (fxToBase === undefined) throw new Error(`Missing FX rate for ${symbol} at ${candle.ts.toISOString()}`);
      const priceMultiplier = this.priceMultiplierForSymbol(symbol);
      const oldQtyAbs = existing ? Math.abs(existing.quantity) : 0;
      const newQtyAbs = Math.abs(qty);
      const totalQtyAbs = oldQtyAbs + newQtyAbs;
      const averageCost = totalQtyAbs > 0
        ? (((existing?.averageCost ?? 0) * oldQtyAbs) + fillPrice * newQtyAbs) / totalQtyAbs
        : fillPrice;
      const oldLocalNotional = existing ? Math.abs(existing.averageCost * oldQtyAbs * existing.priceMultiplier) : 0;
      const newLocalNotional = Math.abs(fillPrice * newQtyAbs * priceMultiplier);
      const fxToBaseAtEntry = oldLocalNotional + newLocalNotional > 0
        ? (((existing?.fxToBaseAtEntry ?? fxToBase) * oldLocalNotional) + fxToBase * newLocalNotional) /
    ...
    await this.closePosition(existing, fillPrice, candle.ts, 'opposite_signal', pending.id);
  }
  ...
  private async processBracketExit(candle: Candle): Promise<void> {
    ...
    const stopTouched = position.stop !== undefined
      ? isLong ? candle.low <= position.stop : candle.high >= position.stop
      : false;
    const takeProfitTouched = position.takeProfit !== undefined
      ? isLong ? candle.high >= position.takeProfit : candle.low <= position.takeProfit
      : false;

    if (stopTouched) {
      await this.closePosition(position, Number(position.stop), candle.ts, 'stop', position.orderId);
      return;
    }

    if (takeProfitTouched) {
      await this.closePosition(position, Number(position.takeProfit), candle.ts, 'take_profit', position.orderId);
    }
  }

  private async closePosition(position: Position, exitPrice: number, exitAt: Date, exitReason: string, orderId: number): Promise<void> {
    ...
    if (exitFxToBase === undefined) throw new Error(`Missing FX rate for ${position.symbol} at ${exitAt.toISOString()}`);
    const priceMultiplier = position.priceMultiplier;
    const entryNotional = Math.abs(position.averageCost * quantityAbs * priceMultiplier * position.fxToBaseAtEntry);
    const exitNotional = Math.abs(exitPrice * quantityAbs * priceMultiplier * exitFxToBase);
    const grossPnl = position.quantity > 0
      ? (exitPrice - position.averageCost) * quantityAbs * priceMultiplier * exitFxToBase
      : (position.averageCost - exitPrice) * quantityAbs * priceMultiplier * exitFxToBase;
    const commission = ((entryNotional + exitNotional) * this.options.commissionBps) / 10000;
  ...
  private async closeOpenPositionsAtDatasetEnd(): Promise<void> {
    for (const position of Array.from(this.positions.values())) {
      const price = this.latestPrice(position.symbol.toUpperCase()) ?? position.averageCost;
      const lastTs = this.latestCandleTs(position.symbol.toUpperCase()) ?? this.currentTime ?? new Date();
      await this.closePosition(position, price, lastTs, 'dataset_end', position.orderId);
    }
  }
  ...
  private updateStrategyRuntime(strategyId: string, netPnl: number, exitedAt: Date, exitReason: string, pnlPct: number): void {
    ...
    if (state.cooldownCount >= 1) {
      ...
      state.cooldownUntil = new Date(exitedAt.getTime() + this.options.strategyCooldownMs);
  ...
  private latestPrice(symbol: string): number | undefined {
    ...
    if (currentIndex !== undefined && rows[currentIndex]) return rows[currentIndex].close;
  ...
  private currentFxToBaseByCurrency(): Record<string, number> {
    ...
    for (const currency of new Set(Object.values(this.options.currencyBySymbol).map((value) => value.trim().toUpperCase()))) {
  ...
  private fxToBaseForCurrency(currency: string, at: Date): number | undefined {
    ...
    for (const rate of this.data.fxRates) {
      if (rate.baseCurrency !== baseCurrency || rate.quoteCurrency !== normalizedCurrency) continue;
...
export async function runIsolatedStrategyBacktest(
  ...
  for (let profileIndex = 0; profileIndex < profiles.length; profileIndex += 1) {
    ...
    onProgress?.(`isolated strategy ${profile.id} completed trades=${metrics.trades} pnl=${metrics.totalPnl.toFixed(2)}`);
  }
...
export async function runParallelIsolatedStrategyBacktest(
  ...
  const label = () => {
    ...
    return active.length > 0 ? `active: ${active.join(', ')}` : 'starting strategy lab';
  };

  const publishProgress = async (): Promise<void> => {
    const current = Array.from(progressByStrategy.values()).reduce((sum, value) => sum + value, 0);
    await repo.updateRunProgress(runId, {
      current,
      total: totalEvents,
      label: label()
  ...
  const runProfile = (item: StrategyWorkItem): Promise<{ totalPnl: number; trades: number; wins: number; winRate: number }> => {
    return new Promise((resolve, reject) => {
      ...
      let metrics: { totalPnl: number; trades: number; wins: number; winRate: number } | undefined;
      ...
      worker.on('exit', (code) => {
        ...
        if (code !== 0) {
          reject(new Error(`Strategy worker ${profile.id} exited with code ${code}`));
          return;
        }
        if (!metrics) {
          reject(new Error(`Strategy worker ${profile.id} exited without metrics`));
          return;
        }
        resolve(metrics);
      });
