...
export class HistoricalClient {
  ...
  async fetchHistorical1mRange(
    ...
    while (end.getTime() > dateFrom.getTime()) {
      const chunkStart = new Date(Math.max(dateFrom.getTime(), end.getTime() - chunkMs));
      const durationDays = Math.max(1, Math.ceil((end.getTime() - chunkStart.getTime()) / (24 * 60 * 60 * 1000)));
      const candles = await this.requestHistorical1mChunkWithRetry(sub, end, `${durationDays} D`);
      const filtered = candles.filter((candle) => candle.ts >= dateFrom && candle.ts <= dateTo);
      all.push(...filtered);
      if (filtered.length > 0) await onChunk?.(filtered);
      this.onLog(`historical ${sub.symbol}: ${candles.length} candles for chunk ending ${end.toISOString()}`);
    ...
    return Array.from(unique.values()).sort((a, b) => a.ts.getTime() - b.ts.getTime());
  }

  private async requestHistorical1mChunkWithRetry(sub: InstrumentSubscription, end: Date, durationStr: string): Promise<Candle[]> {
    const attempts = 3;
  ...
  private async requestContractDetails(symbol: string, contractLike: ContractShape): Promise<ContractDetailsShape> {
    ...
    return new Promise<ContractDetailsShape>((resolve, reject) => {
      ...
      const onContractDetailsEnd = (incomingReqId: number) => {
        ...
        cleanup();
        firstDetails ? resolve(firstDetails) : reject(new Error(`No contract details for ${symbol}`));
      };
  ...
  private async requestHistorical1mChunk(sub: InstrumentSubscription, end: Date, durationStr: string): Promise<Candle[]> {
    const reqId = this.allocReqId();
    ...
    return new Promise<Candle[]>((resolve, reject) => {
      ...
      const onError = (err: Error, code?: number, incomingReqId?: number) => {
        if (incomingReqId !== reqId) return;
        cleanup();
        reject(new Error(`historicalData error ${code ?? 'unknown'} for ${sub.symbol}: ${err?.message ?? 'unknown'}`));
      };

      this.ib.on('historicalData', onHistoricalData);
      this.ib.on('error', onError);
      this.ib.reqHistoricalData(reqId, contract, formatIbEndDateTime(end), durationStr, '1 min', 'TRADES', 0, 2, false);
    });
  }
  ...
  private bindListeners(): void {
    this.ib.on('connected', () => this.onLog('TWS socket connected event received'));
    this.ib.on('disconnected', () => {
      this.connected = false;
      this.onLog('TWS socket disconnected');
    });
    this.ib.on('error', (err: Error, code?: number, reqId?: number) => {
      this.onLog(`TWS error code=${code ?? 'n/a'} reqId=${reqId ?? 'n/a'}: ${err?.message ?? err}`);
    });
  }
}
