...
const schema = z.object({
  ...
  POSTGRES_ADMIN_URL: z.string().default('postgresql://postgres:postgres@localhost:5432/postgres'),
  BACKTEST_POSTGRES_URL: z.string().default('postgresql://postgres:postgres@localhost:5432/ikbr_trader_backtest'),
  ...
  BACKTEST_STRATEGY_LAB_CONCURRENCY: z.coerce.number().int().min(1).max(4).default(2)
});
...
function parseWatchlistContractOverrides(raw: string, symbols: string[]): Map<string, Omit<WatchlistInstrument, 'symbol'>> {
...
function buildWatchlistInstruments(): WatchlistInstrument[] {
  ...
  const overrides = parseWatchlistContractOverrides(env.WATCHLIST_CONTRACT_OVERRIDES, symbols);
...
function buildCurrencyBySymbol(instruments: WatchlistInstrument[], baseCurrency: string): Record<string, string> {
  const out: Record<string, string> = {};
  for (const instrument of instruments) {
    out[instrument.symbol.toUpperCase()] = (instrument.currency ?? baseCurrency).trim().toUpperCase();
  }
  return out;
}
...
function parseAssetClassOverrides(raw: string): Record<string, 'stock' | 'commodity' | 'index'> {
  ...
  for (const entry of raw.split(',').map((value) => value.trim()).filter(Boolean)) {
    ...
    if (normalized !== 'stock' && normalized !== 'commodity' && normalized !== 'index') continue;
...
export const config = {
  ...
  currencyBySymbol: buildCurrencyBySymbol(watchlistInstruments, env.IB_CURRENCY),
  ...
  fractionalSymbols: new Set(
    env.SIGNAL_FRACTIONAL_SYMBOLS.split(',').map((s) => s.trim().toUpperCase()).filter(Boolean)
  )
};
