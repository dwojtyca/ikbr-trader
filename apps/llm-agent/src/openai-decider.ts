...
export class OpenAiDecider {
  ...
  async decide(context: DecisionContext): Promise<LlmDecision> {
    ...
    const systemPrompt = [
      ...
      "- Use evidence.coverage to distinguish available sources from UNAVAILABLE, EMPTY or UNVERIFIED_IDENTITY. Never claim financial statements, earnings, macro or broader market trends were researched when unavailable.",
      "- Symbol-only news does not prove the exchange or issuer identity. Excluded or unverified news cannot support the proposed instrument.",
      "- Order prices use evidence.instrument.currency when verified. Account metrics may use another or unverified valuation currency. Never compare or sum different currencies without explicit verified FX evidence; deterministic execution risk remains authoritative.",
