...
function sign(value: number | undefined, deadZone = 0): -1 | 0 | 1 {
  if (value === undefined || !Number.isFinite(value) || Math.abs(value) <= deadZone) return 0;
  return value > 0 ? 1 : -1;
}
...
function scoreTimeframe(
  ...
  if (macdSign !== 0) {
    ...
    reasons.push(`${timeframe}: MACD histogram ${macdSign > 0 ? 'positive' : 'negative'}`);
  }

  const returnSign = sign(snapshot.return20Pct, 0.15);
  if (returnSign !== 0) {
    score += returnSign * weight * 0.25;
    reasons.push(`${timeframe}: return20 ${returnSign > 0 ? 'positive' : 'negative'}`);
  }
...
function currentTimeframeSnapshot(price: number, indicators: IndicatorSnapshot): TimeframeIndicatorSnapshot {
...
export class MarketRegimeDetector {
  detectDetailed(secType: SecType, price: number, indicators: IndicatorSnapshot): RegimeAnalysis {
    ...
    const timeframeTrendVotes: TimeframeTrendVotes = { bullish: 0, bearish: 0, neutral: 0 };
    for (const item of timeframeScores) {
      timeframeTrendScores[item.timeframe] = round(item.score);
      timeframeTrendVotes[item.vote] += 1;
    }

    const majorVotes = timeframeScores.filter((item) => item.timeframe === '1h' || item.timeframe === '4h' || item.timeframe === '1d' || item.timeframe === '1w');
    const majorBullish = majorVotes.filter((item) => item.vote === 'bullish').length;
    const majorBearish = majorVotes.filter((item) => item.vote === 'bearish').length;
    ...
    if (score >= thresholds.directionalScore && majorBullish >= 2) {
      ...
      reasons.push(`direction bull_trend: score=${round(score)}, majorBullish=${majorBullish}`);
    } else if (score <= -thresholds.directionalScore && majorBearish >= 2) {
      directionalRegime = 'bear_trend';
      reasons.push(`direction bear_trend: score=${round(score)}, majorBearish=${majorBearish}`);
    } else {
    ...
    const confirmationConfidence = majorVotes.length > 0
      ...
      : 0;
    const confidence = clamp(directionalConfidence * 0.65 + confirmationConfidence * 0.35, 0, 1);

    const strongestTimeframes = [...timeframeScores]
      .sort((a, b) => Math.abs(b.score) - Math.abs(a.score))
      .slice(0, 3);
    for (const item of strongestTimeframes) {
      reasons.push(`${item.timeframe} score=${round(item.score)} vote=${item.vote}`);
    }
