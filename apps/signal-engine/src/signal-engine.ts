...
import { ExposureSnapshot, SignalPerformanceStats, SignalRepository } from './repository.js';
...
const STRATEGY_ID = 'momentum_breakout_long_v1';
...
export class SignalEngine {
  ...
  private readonly strategy = new MomentumBreakoutLongStrategy();
  ...
  async runForSymbol(symbol: string, exposureSnapshot?: ExposureSnapshot, generatedFromCandleTs?: Date): Promise<ProposedOrder> {
    ...
    const [candles5m, candles4h, candles12h, candles1d, candles1w] = await Promise.all([
    ...
    if (candles.length < this.options.minCandles) {
      return this.rejectedOrder(symbol, undefined, 'Insufficient candles for indicators', undefined, 'HOLD', generatedFromCandleTs);
    }
    ...
    const trendFrom1h = lastEma(candles1h.map((candle) => candle.close), 50);
    ...
    const runtimeState = await this.repo.getStrategyRuntimeState(STRATEGY_ID);
    if (!runtimeState.enabled || runtimeState.permanentlyDisabled) {
      return this.rejectedOrder(symbol, latest.conid, `${STRATEGY_ID} is disabled`, indicators, 'HOLD', generatedFromCandleTs);
    }
    if (runtimeState.cooldownUntil && runtimeState.cooldownUntil.getTime() > Date.now()) {
      return this.rejectedOrder(symbol, latest.conid, `${STRATEGY_ID} is in cooldown`, indicators, 'HOLD', generatedFromCandleTs);
    }

    const marketState = await this.repo.getMarketState(latest.conid);
    if (!marketState) {
      return this.rejectedOrder(symbol, latest.conid, 'No market state in Redis for conid', indicators, 'HOLD', generatedFromCandleTs);
    }
    ...
    if (this.options.volumeFilterMode === 'strict' && latest.volume < this.options.minVolume1m) {
      return this.rejectedOrder(symbol, latest.conid, 'Liquidity filter rejected: low 1m volume', indicators, 'HOLD', generatedFromCandleTs);
    }
  ...
  private async buildOrderFromSignal(input: {
    ...
    const { symbol, latest, indicators, signal, marketState, riskSnapshot, existingPositionQty, positionMarketValue, spreadBps, generatedFromCandleTs } = input;
    ...
    if (increasesExistingExposure) {
      return this.rejectedOrder(symbol, latest.conid, 'Position already open in signal direction', indicators, signal.side, generatedFromCandleTs);
    }
    ...
    const entrySelection = this.selectEntryPrice(signal.side, latest.close, marketState);
    const entry = signal.suggestedEntry ?? entrySelection.entry;
    let stop = signal.stopLoss ?? (signal.side === 'BUY' ? entry - indicators.atr14! * this.options.atrStopMult : entry + indicators.atr14! * this.options.atrStopMult);
    let takeProfit = signal.takeProfit ?? (signal.side === 'BUY' ? entry + indicators.atr14! * this.options.atrTpMult : entry - indicators.atr14! * this.options.atrTpMult);
    ...
    const minStopBps = Math.max(0, this.options.minStopBpsByAssetClass[indicators.assetClass ?? 'stock'] ?? 0);
    const minRiskPerUnit = entry * (minStopBps / 10000);
    if (minRiskPerUnit > rawRiskPerUnit) {
      stop = signal.side === 'BUY' ? entry - minRiskPerUnit : entry + minRiskPerUnit;
      takeProfit = signal.side === 'BUY' ? entry + minRiskPerUnit * rr : entry - minRiskPerUnit * rr;
    }

    const riskPerUnit = Math.abs(entry - stop);
    if (riskPerUnit <= 0) {
      return this.rejectedOrder(symbol, latest.conid, 'Risk per unit is zero', indicators, signal.side, generatedFromCandleTs);
    }
    ...
    if (fxToBase === undefined) {
      return this.rejectedOrder(symbol, latest.conid, `FX rejected: no conversion rate for ${this.currencyForSymbol(symbol)} to account base`, indicators, signal.side, generatedFromCandleTs);
    }
    ...
    if (notionalEntry <= 0 || riskPerUnitCash <= 0) {
      return this.rejectedOrder(symbol, latest.conid, 'Notional-adjusted risk per unit is zero', indicators, signal.side, generatedFromCandleTs);
    }
    ...
    const riskBasedQuantity = this.roundDownToQuantityStep(maxRiskCash / riskPerUnitCash, quantityStep);
    if (riskBasedQuantity < quantityStep) {
      return this.rejectedOrder(symbol, latest.conid, `Sizing rejected: quantity below minimum step ${quantityStep}`, indicators, signal.side, generatedFromCandleTs);
    }
    ...
    const currentDirectionalExposure = signal.side === 'BUY'
    ...
    if (closesOrReducesPosition) {
      ...
      quantity = this.roundDownToQuantityStep(Math.abs(existingPositionQty), quantityStep);
      if (quantity < quantityStep) {
        return this.rejectedOrder(symbol, latest.conid, `Sizing rejected: no closeable quantity for open position=${existingPositionQty.toFixed(4)}`, indicators, signal.side, generatedFromCandleTs);
      }
    } else {
      if (riskSnapshot.openPositions >= this.options.riskLimits.maxOpenPositions) {
        return this.rejectedOrder(symbol, latest.conid, 'Risk check failed: max open positions reached', indicators, signal.side, generatedFromCandleTs);
      }
      ...
      quantity = this.roundDownToQuantityStep(
        Math.min(riskBasedQuantity, quantityCapByExposure, quantityCapByNotional, quantityCapByDirectionalExposure, quantityCapBySymbolExposure),
        quantityStep
      );
      ...
      if (currentDirectionalExposure + newNotional > maxDirectionalExposure) {
        return this.rejectedOrder(symbol, latest.conid, `Risk overlay rejected: directional exposure too high (current=${currentDirectionalExposure.toFixed(2)}, new=${newNotional.toFixed(2)}, limit=${maxDirectionalExposure.toFixed(2)})`, indicators, signal.side, generatedFromCandleTs);
      }
      if (currentSymbolExposure + newNotional > maxSymbolExposure) {
        return this.rejectedOrder(symbol, latest.conid, `Risk overlay rejected: symbol concentration too high (current=${currentSymbolExposure.toFixed(2)}, new=${newNotional.toFixed(2)}, limit=${maxSymbolExposure.toFixed(2)})`, indicators, signal.side, generatedFromCandleTs);
      }
      if (riskSnapshot.exposure + newNotional > maxExposure) {
        return this.rejectedOrder(symbol, latest.conid, `Risk check failed: max exposure exceeded (current=${riskSnapshot.exposure.toFixed(2)}, new=${newNotional.toFixed(2)}, post=${(riskSnapshot.exposure + newNotional).toFixed(2)}, limit=${maxExposure.toFixed(2)}, source=${riskSnapshot.source})`, indicators, signal.side, generatedFromCandleTs);
      }
    }

    const profilePerformance = await this.repo.getSignalPerformance({ strategy: STRATEGY_ID, side: signal.side, limit: 40 });
    const symbolPerformance = await this.repo.getSignalPerformance({ instrument: symbol, strategy: STRATEGY_ID, side: signal.side, limit: 30 });
    const confidenceFloor = this.buildConfidenceFloor(this.options.minConfidence, profilePerformance, symbolPerformance);
    const spreadScore = clamp(1 - safeDiv(spreadBps, this.options.maxSpreadBps, 0), 0, 1);
    const confidence = clamp(signal.confidenceScore * (0.85 + 0.15 * spreadScore), 0, 1);
    if (confidence < confidenceFloor.value) {
      return this.rejectedOrder(
        symbol,
        latest.conid,
        `Confidence too low (${confidence.toFixed(2)} < ${confidenceFloor.value.toFixed(2)}) for ${STRATEGY_ID}${confidenceFloor.reason ? `; ${confidenceFloor.reason}` : ''}`,
        indicators,
        signal.side,
        generatedFromCandleTs
      );
    ...
    return {
      ...
      reason: `${signal.entryReason}, strategy=${STRATEGY_ID}, regime=${indicators.regime}, mode=${positionEffect}, position=${existingPositionQty.toFixed(4)}, entrySource=${entrySelection.source}`,
      confidence,
  ...
  private hasRequiredIndicators(indicators: IndicatorSnapshot): boolean {
    return indicators.ema20 !== undefined &&
  ...
  private selectEntryPrice(side: Exclude<Side, 'HOLD'>, last: number, marketState: { bid?: number; ask?: number; lastPrice: number }): EntryPriceSelection {
    const mode = this.options.lmtEntryMode;
    const bid = marketState.bid;
    const ask = marketState.ask;
    const bufferSign = side === 'BUY' ? 1 : -1;
    const applyBuffer = (price: number) => price * (1 + bufferSign * this.options.lmtEntryBufferBps / 10000);
  ...
  private buildConfidenceFloor(baseMinConfidence: number, profilePerformance: SignalPerformanceStats, symbolPerformance: SignalPerformanceStats): ConfidenceFloor {
