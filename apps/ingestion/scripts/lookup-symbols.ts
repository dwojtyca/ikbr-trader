...
const CANDIDATES: Array<{ label: string; attempts: Attempt[] }> = [
  ...
  ...["NFLX", "UBER", "DIS", "BA", "LLY", "MARA", "RIOT", "BABA",
      "JPM", "GS", "HOOD", "XOM", "CVX"].map((s) => ({
    label: s,
    attempts: [{ symbol: s, secType: "STK", exchange: "SMART", primaryExchange: "NASDAQ", currency: "USD" },
               { symbol: s, secType: "STK", exchange: "SMART", primaryExchange: "NYSE", currency: "USD" },
  ...
  ...["XLE", "XLF", "XLK", "XLV", "DIA", "GLD", "SLV"].map((s) => ({
    label: s,
    attempts: [{ symbol: s, secType: "STK", exchange: "SMART", primaryExchange: "ARCA", currency: "USD" },
               { symbol: s, secType: "STK", exchange: "SMART", currency: "USD" }],
  })),
  ...
  ...["PKN", "PKO", "SPL", "CDR", "JSW", "CCC", "MBK", "TPE"].map((s) => ({
    label: s,
    attempts: [{ symbol: s, secType: "STK", exchange: "WSE", primaryExchange: "WSE", currency: "PLN" }],
  })),
];

interface Detail { conId?: number; symbol?: string; secType?: string; exchange?: string; primaryExch?: string; currency?: string; longName?: string; }
...
const pending = new Map<number, { resolve: Resolver; reject: Rejecter; buf: Detail[]; timer: NodeJS.Timeout; label: string }>();
...
ib.on("error", (err: any, code?: number, reqId?: number) => {
  ...
  } else if (code && code !== 2104 && code !== 2106 && code !== 2158 && code !== 2107 && code !== 2100) {
...
async function run() {
  await new Promise<void>((resolve, reject) => {
    ...
    ib.once("connected", () => { clearTimeout(t); resolve(); });
    ib.on("server", () => {}); // noop
    ib.connect();
    setTimeout(() => { clearTimeout(t); resolve(); }, 1500); // ib package fires connect synchronously sometimes
  });
  ...
  for (const c of CANDIDATES) {
    ...
    for (const a of c.attempts) {
      try {
        ...
        if (details.length > 0) {
          ...
          resolved = details.find((d) => d.currency === a.currency) ?? details[0];
    ...
    if (resolved) {
      ...
      console.log(`${c.label}\tNOT_FOUND\t-\t-\t-\t-\t${lastErr || "no match"}`);
    }
  }
