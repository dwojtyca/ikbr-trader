...
function parseRtVolumeTick(raw: string): { price?: number; size?: number } | undefined {
...
export class TwsClient {
  ...
  async connect(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const onError = (err: Error, code?: number) => {
        if (code === 502 || code === 503 || code === 504) {
          cleanup();
          reject(new Error(`TWS socket connection failed (${code}): ${err.message}`));
        }
      };
    ...
    this.onLog(`TWS socket connected ${this.config.host}:${this.config.port}, clientId=${this.config.clientId}`);
    try {
  ...
  async resolveContracts(instruments: WatchlistInstrument[]): Promise<InstrumentSubscription[]> {
    ...
    for (const instrument of instruments) {
      try {
        ...
        this.onLog(`resolve contract failed for ${instrument.symbol}: ${(error as Error).message}`);
      }
    }
  ...
  addSubscriptions(subscriptions: InstrumentSubscription[]): void {
    for (const sub of subscriptions) {
      ...
      const contract = this.withDefaults(sub.contract ?? { symbol: sub.symbol, conId: Number(sub.conid) });
  ...
  async backfillRecentCandles1m(subscriptions: InstrumentSubscription[], candlesPerSymbol: number): Promise<HistoricalBackfillResult[]> {
    ...
    for (const sub of subscriptions) {
      try {
        const candles = await this.requestHistorical1m(sub, candlesPerSymbol);
        out.push({
          symbol: sub.symbol,
          conid: sub.conid,
          candles
        ...
        this.onLog(`historical backfill failed for ${sub.symbol} (${sub.conid}): ${(error as Error).message}`);
        out.push({
          symbol: sub.symbol,
          conid: sub.conid,
          candles: []
  ...
  private async requestContractDetails(symbol: string, contractLike: ContractShape): Promise<ContractDetailsShape> {
    ...
    return new Promise<ContractDetailsShape>((resolve, reject) => {
      ...
      const onError = (err: Error, code?: number, incomingReqId?: number) => {
        if (incomingReqId !== reqId) return;
        cleanup();
        reject(new Error(`contractDetails error ${code ?? 'unknown'} for ${symbol}: ${err.message}`));
      };
  ...
  private buildContractFromInstrument(instrument: WatchlistInstrument, directConid?: number): ContractShape {
    return {
      ...
      ...(instrument.primaryExchange ? { primaryExch: instrument.primaryExchange } : {}),
      ...(instrument.currency ? { currency: instrument.currency } : {})
    };
  }

  private async requestHistorical1m(sub: InstrumentSubscription, candlesPerSymbol: number): Promise<Candle[]> {
    const reqId = this.allocReqId();
    const durationDays = Math.max(1, Math.min(30, Math.ceil(candlesPerSymbol / 390) + 1));
    const durationStr = `${durationDays} D`;
    const contract = this.withDefaults(sub.contract ?? { symbol: sub.symbol, conId: Number(sub.conid) });
    ...
    return new Promise<Candle[]>((resolve, reject) => {
      ...
      const finalize = () => {
        ...
        const ordered = Array.from(uniqueByTs.values()).sort((a, b) => a.ts.getTime() - b.ts.getTime());
        const sliced = ordered.slice(Math.max(0, ordered.length - candlesPerSymbol));
        this.onLog(`historical backfill ${sub.symbol} (${sub.conid}) fetched ${sliced.length}/${candlesPerSymbol} candles`);
        resolve(sliced);
      };
      ...
      const onHistoricalData = (
        ...
        if (typeof date === 'string' && date.toLowerCase().startsWith('finished')) {
          finalize();
          return;
        }
      ...
      const onError = (err: Error, code?: number, incomingReqId?: number) => {
        ...
        cleanup();
        reject(new Error(`historicalData error ${code ?? 'unknown'} for ${sub.symbol}: ${message}`));
      };
  ...
  private bindCoreListeners(): void {
    ...
    this.ib.on('error', (arg1: unknown, arg2?: unknown, arg3?: unknown) => {
      ...
      this.onLog(`TWS error code=${parsed.code ?? 'n/a'} ${prefix}: ${parsed.message}`);
    });
    ...
    this.ib.on('tickSize', (tickerId: number, field: number, size: number) => {
      ...
      if (field === 0 || field === 3 || field === 5 || field === 69 || field === 70 || field === 71) {
        state.size = size;
      }
    ...
    this.ib.on('tickString', (tickerId: number, field: number, value: string) => {
      const state = this.tickerStates.get(tickerId);
      if (!state) return;
