-- PR14.2 — versioned baseline.
--
-- IDENTICAL to `infra/sql/001_init.sql`. `001_init.sql` is loaded on
-- fresh Docker Postgres via `docker-entrypoint-initdb.d`; this
-- migration recreates the same schema on any database that skipped
-- that path (hermetic tests, CI, manual dumps restored on top of an
-- empty cluster). Every statement is idempotent so both routes can
-- run against the same database without conflict.
--
-- DO NOT EDIT AFTER APPLYING. If the schema needs to change, add a
-- new migration file — the runner enforces this via checksum
-- comparison against `schema_migrations`.
